...
class FeatureExtractor():
    def __init__(self,query_df,item_cats='item_categories.csv',item='items.csv',shops='shops.csv',train='sales_train.csv') -> None:
        self.jarr=query_df
        self.item_cats=pd.read_csv(item_cats)
        self.item=pd.read_csv(item)
        self.shops=pd.read_csv(shops)
        self.train=pd.read_csv(train, parse_dates=['date'], dtype={'date': 'str', 'date_block_num': 'int32', 'shop_id': 'int32','item_id': 'int32', 'item_price': 'float32', 'item_cnt_day': 'int32'})
        
        return None

    def transforms(self):
        t=self.jarr
        sales_train=self.train
        sales_item=self.item
        sales_item_category=self.item_cats
        sales_shops=self.shops
        ...
        # sales_dataset = sales_dataset[sales_dataset['shop_id'].isin(t['shop_id'].unique())]
        # sales_dataset = sales_dataset[sales_dataset['item_id'].isin(t['item_id'].unique())]
        ...
        sales_train_new = pd.merge(t,sales_train_new,on = ['item_id','shop_id'],how = 'left')

        sales_train_new.fillna(0,inplace = True)
        sales_train_new=sales_train_new.drop(0,axis=1)
        sales_train_new.to_csv('sales_train_for_pre.csv')
        
        return sales_train_new